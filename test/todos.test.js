...
module.exports = function ( nodoovent, url ) {
	...
	describe ( "Test /todos, /user/:userid/todos and /todos/:id end points:", function ( ) {
		...
		describe ( "/todos/:id", function ( ) {
			...
			describe ( "DELETE /todos/:id with oauth1 authentication", function ( ) {
				...
				it ( "delete todo authenticated user aren't author", function ( callback ) {
					Todos.findOne ( ).where ( { author: { "!": peterpan.id } } ).exec ( function ( err, todo ) {
						if ( err ) callback ( err );
						oauth.delete ( url + "/todos/" + todo.id, accesstoken, accesssecret, function ( err, data, res ) {
							res.should.have.status ( 403 );
							callback ( );
						} );
					} );
				...
				it ( "delete todo authenticated user are the author", function ( callback ) {
					Todos.findOne ( ).where ( { author: peterpan.id } ).exec ( function ( err, todo ) {
						if ( err ) callback ( err );
						oauth.delete ( url + "/todos/" + todo.id, accesstoken, accesssecret, function ( err, data, res ) {
							res.should.have.status ( 200 );
							...
							Todos.find ( ).where ( { id: todo.id } ).exec ( function ( err, todo ) {
